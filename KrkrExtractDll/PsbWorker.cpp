...
{
	ULONG nRet = 0;
	WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), Info, lstrlenW(Info), &nRet, NULL);
}
		...
		DataLength(len)
	{
		...
		else
		{
				...
				data[(ULONG_PTR)pInfo->pRootCode] != 0x21)
			{
				...
				{
					throw std::wstring(L"Unable to get private for this emote file...\n");
				}
			}
...
public:
	...
	PSBDumper(LPWSTR _path)
	{
		...
		std::string::size_type pos = tmp.rfind(L".");
	...
	void dump(const PSBNode &node, const std::string& indentstr = "")
	{
		...
		case PsbValueFloat:
			memset(NumStr, 0, sizeof(NumStr));
			//sprintf(NumStr, "%f", node.AsDouble());
	...
	void start(const PSBNode &node, const std::wstring& FileName)
	{
		...
		{
			//Decompile Script
			std::wstring scriptFile = psbimgpath + L".psb.tjs";
			FILE* fp = _wfopen(scriptFile.c_str(), L"wb");
