...
public:
	...
	const tjs_char *GetContextTypeNameBegin() const;
	const tjs_char *GetContextTypeNameEnd() const;
...
public:
	...
	static tTJSString GetValueComment(const tTJSVariant &val, std::map<tjs_uint, ttstr>& ObjMap);

	void Disassemble(
		void (*output_func)(const tjs_char *msg, const tjs_char *comment,
		tjs_int addr, const tjs_int32 *codestart, tjs_int size, void *data),
		void (*output_func_src)(const tjs_char *msg, const tjs_char *name,
		tjs_int line, void *data), void *data, std::map<tjs_uint, ttstr>& ObjMap, tjs_int start = 0, tjs_int end = 0);
	void Disassemble(void(*output_func)(const tjs_char *msg, void* data), void *data, std::map<tjs_uint, ttstr>& ObjMap,
		tjs_int start = 0, tjs_int end = 0);
	void Disassemble(std::map<tjs_uint, ttstr>& ObjMap, tjs_int start = 0, tjs_int end = 0);
