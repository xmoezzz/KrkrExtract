	...
	tjs_int maxlen)
{
	...
	case tvtVoid:
	  {
		tTJSString str(TJS_W("#Var.Void"));
		TJSTrimStringLength(str, maxlen);
		return str;
	  }
	case tvtInteger:
	  {
		  tTJSString str(TJS_W("#Var.Int("));
		str += (tTJSString)val;
		str += L")";
		TJSTrimStringLength(str, maxlen);
		return str;
	...
	case tvtString:
	  {
		tTJSString str(TJS_W("#Var.String(\""));
	...
	case tvtOctet:
	  {
		...
		tTJSString str(TJS_W("#Var.Octet(<% "));
	...
	case tvtObject:
	  {
		tTJSString str(TJS_W("#Var.Object("));
		try
		{
			str += ttstr(val);
		}
...
tTJSString TJSVariantToReadableString(const tTJSVariant &val, std::map<tjs_uint, ttstr>& ObjMap,
	tjs_int maxlen)
{
	// convert given variant to human-readable string
	// ( eg. "(string)\"this is a\\nstring\"" )
	...
	case tvtObject:
	{
		...
		try
		{
			str += ObjMap[(tjs_uint) (*(tTJSVariantClosure*)&(val.Object)).Object];
		}
		catch (...)
		{
		}
