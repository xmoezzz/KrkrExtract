...
template<class T>
std::shared_ptr<T> my_malloc(DWORD Size)
{
...
)
{
	...
	auto output = my_malloc<BYTE>(output_length);
	if (!output)
		return -1;

	bmfh = (BITMAPFILEHEADER *)output.get();
	bmfh->bfType = 0x4D42;
...
int
tlg6_process(
std::shared_ptr<BYTE> tlg_raw_data,
DWORD tlg_size,
std::shared_ptr<BYTE>& ret_actual_data,
DWORD& ret_actual_data_length
)
{
	tlg6_header_t *tlg6_header = (tlg6_header_t *)tlg_raw_data.get();
	if (tlg_size <= sizeof(tlg6_header_t))
		return -1;

	int outsize;

	outsize = tlg6_header->width * 4 * tlg6_header->height;
	std::shared_ptr<BYTE> out = my_malloc<BYTE>(outsize);
	if (!out)
		return -1;
	...
	std::shared_ptr<BYTE>  bit_pool = my_malloc<BYTE>((tlg6_header->max_bit_length / 8 + 5 + 3) & ~3);
	std::shared_ptr<ULONG> pixelbuf = my_malloc<ULONG>((4 * tlg6_header->width * TVP_TLG6_H_BLOCK_SIZE + 1 + 3) & ~3);
	std::shared_ptr<BYTE>  filter_types = my_malloc<BYTE>((x_block_count * y_block_count + 3) & ~3);
	std::shared_ptr<ULONG> zeroline = my_malloc<ULONG>(tlg6_header->width * 4);
	...
	TVPFillARGB(zeroline.get(), tlg6_header->width,
	...
	TVPTLG5DecompressSlide(filter_types.get(), inbuf, inbuf_size, LZSS_text, 0);

	BYTE *in_p = inbuf + inbuf_size;
	// for each horizontal block group ...
	ULONG *prevline = zeroline.get();
	for (int y = 0; y < (int)tlg6_header->height; y += TVP_TLG6_H_BLOCK_SIZE) {
		...
		for (int c = 0; c < tlg6_header->colors; c++) {
			...
			CopyMemory(bit_pool.get(), in_p, byte_length);
			in_p += byte_length;
			...
			case 0:
				if (c == 0 && tlg6_header->colors != 1)
					TVPTLG6DecodeGolombValuesForFirst((char *)pixelbuf.get(),
					pixel_count, bit_pool.get());
				else
					TVPTLG6DecodeGolombValues((char *)pixelbuf.get() + c,
					pixel_count, bit_pool.get());
				break;
		...
		// for each line
		unsigned char *ft =
			filter_types.get() + (y / TVP_TLG6_H_BLOCK_SIZE) * x_block_count;
		int skipbytes = (ylim - y) * TVP_TLG6_W_BLOCK_SIZE;

		BYTE* pout = out.get();
		for (int yy = y; yy < ylim; yy++) {
			ULONG *curline = (ULONG *)&pout[yy * tlg6_header->width * 4];
			...
			if (main_count) {
				...
				TVPTLG6DecodeLine(
					...
					ft,
					skipbytes,
					pixelbuf.get() + start, tlg6_header->colors == 3 ? 0xff000000 : 0,
					oddskip, dir);
			}
	...
	if (MyBuildBMPFile(out.get(), outsize, NULL, 0, tlg6_header->width,
		0 - tlg6_header->height, 32, ret_actual_data,
		ret_actual_data_length))
	{
		return -1;
	}
...
)
{
	...
	return !tlg6_process(InBuffer, uInSize, OutBuffer, OutSize);
}
	...
	ULONG& OutSize)
{
	...
	if (uInSize <= sizeof(KRKR2_TLG5_HEADER))
		return FALSE;
	...
	OutBuffer = my_malloc<BYTE>(stride * height + sizeof(BitmpHeader));
	if (OutBuffer == NULL)
		return FALSE;
	
	pbOutBuffer = OutBuffer.get();
	OutSize = stride * height + sizeof(BitmpHeader);
	*(IMAGE_BITMAP_HEADER*)pbOutBuffer = BitmpHeader;
	...
	// decomperss
	std::shared_ptr<BYTE> outbuf[4];
	UCHAR text[0x1000];
	int r = 0;

	memset(text, 0, 4096);

	auto inbuf = my_malloc<BYTE>(blockheight * width + 10);
	for (int i = 0; i < colors; i++)
	{
		outbuf[i] = my_malloc<BYTE>(blockheight * width + 10);
		if (!outbuf[i])
		{
			return FALSE;
		}
	...
	for (int y_blk = 0; y_blk < height; y_blk += blockheight)
	{
		...
		for (int c = 0; c < colors; c++)
		{
			...
			if (mark[0] == 0)
			{
				...
				CopyMemory(inbuf.get(), pbInBuffer, size);
				pbInBuffer += size;
				r = TVPTLG5DecompressSlide(outbuf[c].get(), inbuf.get(), size, text, r);
			}
			else
			{
				// raw data
				CopyMemory(outbuf[c].get(), pbInBuffer, size);
				pbInBuffer += size;
			}
		}
		...
		UCHAR * outbufp[4];
		for (int c = 0; c < colors; c++) outbufp[c] = outbuf[c].get();
