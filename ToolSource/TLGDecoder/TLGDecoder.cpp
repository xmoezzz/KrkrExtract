...
)
{
	...
	ULONG *pixelbuf = (ULONG *)mem.Alloc((4 * tlg6_header->width * TVP_TLG6_H_BLOCK_SIZE + 1 + 3) & ~3);
	BYTE *filter_types = (BYTE *)mem.Alloc((x_block_count * y_block_count + 3) & ~3);
	ULONG *zeroline = (ULONG *)mem.Alloc(tlg6_header->width * 4);
...
BOOL DecodeTLG5(PVOID lpInBuffer, ULONG uInSize, PVOID *ppOutBuffer, PULONG pOutSize)
{
	...
	*(SBitMapHeader *)pbOutBuffer = BitmpHeader;
	...
	text = (UCHAR*)mem.Alloc(4096);
	memset(text, 0, 4096);

	inbuf = (UCHAR*)mem.Alloc(blockheight * width + 10);
	for (int i = 0; i < colors; i++)
		outbuf[i] = (UCHAR*)mem.Alloc(blockheight * width + 10);
